...
EN_terminalError_t getTransactionDate(ST_terminalData_t* termData) {
  ...
  fgets(date, 11, stdin);
  clean(date);
  if (!isValidTransactionDate(date)) return WRONG_DATE;
...
EN_terminalError_t getTransactionAmount(ST_terminalData_t* termData) {
  ...
  scanf("%f", &amount);
  if (!isValidTransactionAmount(amount)) return INVALID_AMOUNT;
...
EN_terminalError_t isBelowMaxAmount(ST_terminalData_t* termData) {
  if (!acceptableAmount(termData->transAmount, termData->maxTransAmount)) return EXCEED_MAX_AMOUNT;
  return TERMINAL_OK;
  }
...
EN_terminalError_t setMaxAmount(ST_terminalData_t* termData, float maxAmount) {
  if (!isValidMaxTransactionAmount(maxAmount)) return INVALID_MAX_AMOUNT;
...
void getTransactionAmountTest(void) {
  ...
  for (uint8_t i = 0;i < 3;i++) {
    ...
    printf("Expected result: %s\n", expected_output[i == 2]);
    printf("Actual result: %s\n", expected_output[isValidTransactionAmount(test_cases[i]) == TERMINAL_OK]);
    printf("\n");
    }
  }
...
void isBelowMaxAmountTest(void) {
  ...
  for (uint8_t i = 0;i < 2;i++) {
    ...
    printf("Input Data : %0.2f\n", test_cases[i]);
    printf("Expected result: %s\n", expected_output[i]);
    printf("Actual result: %s\n", expected_output[acceptableAmount(test_cases[i], max) == TERMINAL_OK]);
    printf("\n");
    }
  }
...
void setMaxAmountTest(void) {
  ...
  for (uint8_t i = 0;i < 3;i++) {
    ...
    printf("Input Data : %0.2f\n", test_cases[i]);
    printf("Expected result: %s\n", expected_output[i == 2]);
    printf("Actual result: %s\n", expected_output[isValidMaxTransactionAmount(test_cases[i]) == TERMINAL_OK]);
    printf("\n");
    }
  }